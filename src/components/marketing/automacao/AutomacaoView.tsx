...
export function AutomacaoView() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await apiAvaliacaoStatus();
      setData(res);
      // Só sobrescreve o formulário se o usuário não estiver editando.
  ...
  const patch = (p: Partial<AvaliacaoCampanhaConfig>) => {
    dirty.current = true;
    setCfg((c) => (c ? { ...c, ...p } : c));
  };
  ...
  const salvar = async () => {
    ...
    setMsg(null);
    try {
      await apiAvaliacaoSaveConfig(cfg);
      dirty.current = false;
  ...
  const montarFila = async () => {
    ...
    setMsg(null);
    try {
      const r = await apiAvaliacaoBuild();
      setMsg(`Fila montada: ${r.pending} clientes elegíveis prontos para receber.`);
      await load();
    } catch (e) {
  ...
  const controlar = async (action: "start" | "pause" | "stop") => {
    ...
    setMsg(null);
    try {
      await apiAvaliacaoControl(action);
      await load();
    } catch (e) {
      ...
      <div className="max-w-5xl w-full mx-auto p-6 md:p-8 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-medium">
                Pergunta o que os clientes que compraram acharam do atendimento e pede avaliação no Google.
              </p>
            </div>
          </div>
        ...
        <div className="flex gap-3 p-4 rounded-2xl bg-sky-500/5 border border-sky-500/20">
          ...
          <p className="text-xs text-muted-foreground font-medium leading-relaxed">
            Modo seguro: envia só para <b>clientes convertidos</b> (que já compraram), respeitando
            horário comercial, um <b>intervalo aleatório</b> entre cada mensagem, um <b>teto diário</b>{" "}
            (padrão <b>5/dia</b>) e variações de texto. Quem responder "parar/sair" é removido
            automaticamente.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <Stat icon={Star} label="Total enviados" value={c.sent} color="text-primary" />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <ListPlus className="w-4 h-4" /> Montar fila (clientes convertidos)
          </button>

          {status !== "running" ? (
            <button
              onClick={() => controlar("start")}
        ...
        {cfg && (
          <div className="rounded-2xl border border-border bg-card p-5 space-y-5">
            ...
            <Field label="Link de avaliação (Google)">
              <input type="text" value={cfg.review_url}
                onChange={(e) => patch({ review_url: e.target.value })}
                className={inputCls} placeholder="https://g.page/r/.../review" />
            </Field>

            <Field label="Variações de mensagem (uma por linha) — use {nome} e {link}">
              <textarea
                ...
                placeholder="Oi {nome}! Deixa sua avaliação no Google: {link}"
              />
            </Field>
