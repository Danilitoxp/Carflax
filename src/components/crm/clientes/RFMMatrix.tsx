...
export function RFMMatrix({ data = [], onCellClick }: RFMMatrixProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-12 items-center lg:items-start justify-center p-4 sm:p-8 bg-card border border-border rounded-3xl shadow-xl animate-in fade-in zoom-in duration-500 w-full overflow-hidden">
      {/* Chart Area */}
      <div className="relative flex flex-col items-center pl-6 pb-6 sm:pl-0 sm:pb-0 max-w-full">
        <div className="absolute -left-4 sm:-left-16 top-1/2 -rotate-90 origin-center whitespace-nowrap text-[8px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest">
          Frequência e Valor (média)
        </div>

        <div className="absolute -bottom-4 sm:-bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap text-[8px] sm:text-[10px] font-black text-muted-foreground uppercase tracking-widest">
          Recência
        </div>

        <div className="relative p-1.5 sm:p-2 bg-background border border-border rounded-xl">
          {/* Arrow Y */}
          <div className="absolute -left-3 sm:-left-4 -top-3 sm:-top-4 bottom-0 w-px bg-muted-foreground/30">
          ...
          {/* Arrow X */}
          <div className="absolute -left-3 sm:-left-4 -bottom-3 sm:-bottom-4 right-0 h-px bg-muted-foreground/30">
              ...
              row.map((key, xIdx) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "w-[48px] h-[48px] sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-lg flex flex-col items-center justify-center p-1 sm:p-2 text-center transition-all hover:scale-105 hover:shadow-lg cursor-pointer group relative",
                      cell.color.startsWith("bg-") ? cell.color : ""
                    )}
                    style={!cell.color.startsWith("bg-") ? { backgroundColor: cell.color } : {}}
                  >
                    {xIdx === 0 && (
                      <span className="absolute -left-4 sm:-left-6 top-1/2 -translate-y-1/2 text-[8px] sm:text-[10px] font-bold text-muted-foreground">{yVal}</span>
                    )}
                    {yIdx === 4 && (
                      <span className="absolute left-1/2 -bottom-4 sm:-bottom-6 -translate-x-1/2 text-[8px] sm:text-[10px] font-bold text-muted-foreground">{xVal}</span>
                    )}

                    <span className="text-sm sm:text-base md:text-xl font-black text-white/90 group-hover:scale-110 transition-transform tabular-nums">
                      {count}
                    </span>
                    <span className="hidden sm:block text-[7px] font-black uppercase text-white/60 tracking-tighter mt-1 leading-tight">
                      {count === 1 ? "Cliente" : "Clientes"}
                    </span>

                    {/* Tooltip on Hover */}
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center p-1.5 z-20 pointer-events-none">
                      <span className="text-[7px] sm:text-[8px] font-black text-white uppercase leading-tight">
                        {cell.label}
                      </span>
                    </div>
                  </div>
      ...
      {/* Legend Area */}
      <div className="flex flex-col gap-4 w-full lg:w-auto lg:min-w-[240px] flex-1">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-1 gap-1">
          {LEGEND.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 sm:gap-3 p-1.5 sm:p-2 hover:bg-secondary/50 rounded-xl transition-all cursor-pointer group">
              <div className={cn("w-4 h-4 sm:w-5 sm:h-5 rounded-md shrink-0 shadow-sm transition-transform group-hover:scale-110", item.color)} />
              <span className="text-[8px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-tight group-hover:text-foreground transition-colors truncate">
                {item.label}
              </span>
            </div>
          ))}
        ...
        <div className="mt-2 p-3 sm:p-4 bg-primary/5 border border-primary/10 rounded-2xl">
