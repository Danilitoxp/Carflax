...
WITH fat_base AS (
    SELECT
        ...
        ROUND(
            SUM(
                CASE
                    WHEN DATE(DATA) = ?
                    THEN COALESCE(PRAZO_MEDIO, 0) * TOTAL_VENDA
                    ELSE 0
                END
            ) / NULLIF(
                SUM(
    ...
    WHERE 1=1
    ${vendedor ? 'AND TRIM(COD_VENDEDOR) = ?' : ''}
...
orc AS (
    ...
    WHERE A.FGO_ESPDOC = 'OR'
      AND A.FGO_DTAENT >= ?
      ${vendedor ? 'AND TRIM(A.FGO_CODVEN) = ?' : ''}
...
vendas_hoje AS (
    ...
    WHERE DATE(f1.FGO_DTAORI) = ?
      ${vendedor ? 'AND TRIM(f1.FGO_CODVEN) = ?' : ''}
...
SELECT
    ...
    COALESCE(fat_base.FATURADO, 0)
    + COALESCE(nf_fatgor.NF_FATGOR, 0) AS TOTAL,

    GREATEST(
        COALESCE(meta.META, 0)
        - (
            COALESCE(fat_base.FATURADO, 0)
            + COALESCE(nf_fatgor.NF_FATGOR, 0)
        ),
        0
    ) AS FALTANTE,
    ...
    COALESCE(fat_base.FATURADO, 0)
    - COALESCE(fat_base.CUSTO, 0) AS MARGEM_REAL,

    ROUND(
        (
            COALESCE(fat_base.FATURADO, 0)
            - COALESCE(fat_base.CUSTO, 0)
        ) / NULLIF(COALESCE(fat_base.FATURADO, 0), 0) * 100,
        2
    ) AS MARGEM_REAL_PERC,
...
FROM meta
LEFT JOIN fat_base
    ON fat_base.COD_VENDEDOR = meta.COD_VENDEDOR
LEFT JOIN nf_fatgor
    ON nf_fatgor.COD_VENDEDOR = meta.COD_VENDEDOR
LEFT JOIN orc
    ON orc.COD_VENDEDOR = meta.COD_VENDEDOR
LEFT JOIN vendas_hoje
    ON vendas_hoje.COD_VENDEDOR = meta.COD_VENDEDOR
